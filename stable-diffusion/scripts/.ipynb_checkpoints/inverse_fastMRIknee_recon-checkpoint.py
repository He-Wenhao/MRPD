...
parser.add_argument(
    "--ckpt",
    type=str,
    #default='../../stable-diffusion/models/model/v1-5-pruned-emaonly.ckpt',
    #default='../../stable-diffusion/models/finetune/model.ckpt',
    default = '../../stable-diffusion/models/finetune_val/finetune_val.ckpt',
...
def plot_img(image_tensor,name):
    #image_tensor = self[idx][0]  # Get the image tensor from the dataset
    #image_tensor = image_tensor  # Convert to NumPy array
    # Convert to NumPy array for plotting (matplotlib expects HWC format)
    #image_tensor = image_tensor.reshape(3, 320, 320)
...
def encode_decode(y,model,name):
    z = model.get_first_stage_encoding(model.encode_first_stage(y))
    img_p = model.decode_first_stage(z)
    img_p = torch.clamp((img_p + 1.0) / 2.0, min=0.0, max=1.0)
    img_p = img_p.cpu().permute(0, 2, 3, 1).numpy()  
    img_p = np.mean(img_p[0], axis=-1)
    plot_img(img_p,name)
    return img_p
...
sampler = DDIMSampler(model)#dbgdbg

for ind,img in enumerate(test_dataloader):
    ...
    original_img_test = np.mean((original_image + 1)/2, axis=-1)
    forward_image_test = np.mean((forward_image + 1)/2, axis=-1)
    plot_img(original_img_test,'orig.png')
    plot_img(forward_image_test,'yn.png')
    encode_decode(y,model,"encode_decode_y.png")
    encode_decode(org_image,model,"encode_decode_org.png")
        ...
        with model.ema_scope():
            ...
            for n in trange(opt.n_iter, desc="Sampling"):
                for prompts in tqdm(data, desc="data"):
                    ...
                    encode_decode(x_samples_ddim,model,"encode_decode_ddim.png")
    ...
    ssim_recon, _ = ssim(original_img.astype('float64'), pred_img.astype('float64'), full=True, data_range=data_range)
    ...
    print("========================")
    print("test intermediate")
    print("========================")
    with open('intermediate.txt', 'w') as f:
        f.write(f'psnr   , ssim   , mse   \n')
    psnr_recon_l = []
    ssim_recon_l = []
    mse_recon_l = []
    pred_img_l = []
    for i in range(len(intermediate['x_inter'])):
        img = intermediate['x_inter'][i]
        pred_x0 = intermediate['pred_x0'][i]
        x_samples_ddim = model.decode_first_stage(img)
        x_samples_ddim = torch.clamp((x_samples_ddim + 1.0) / 2.0, min=0.0, max=1.0)
        pred_image = x_samples_ddim.cpu().permute(0, 2, 3, 1).numpy()    
        pred_img = np.mean(pred_image[0], axis=-1) 
        pred_img_l.append(pred_img)
        psnr_recon = psnr(original_img, pred_img)
        data_range = original_img.max() - original_img.min()
        ssim_recon, _ = ssim(original_img.astype('float64'), pred_img.astype('float64'), full=True, data_range=data_range)
        mse_recon = np.abs((original_img-pred_img)).sum()/(320*320)
        psnr_recon_l.append(psnr_recon)
        ssim_recon_l.append(ssim_recon)
        mse_recon_l.append(mse_recon)
        with open('intermediate.txt', 'a') as f:
            f.write(f'psnr: {psnr_recon}, ssim: {ssim_recon}, mse: {mse_recon}\n')

    print("========================")
    print("make plot")
    print("========================")

    #plot_img(original_img,'orig_img.png')
    ...
    plt.subplot(1, 3, 2)
    ind_dm = 400
    plt.imshow(pred_img_l[ind_dm])
    plt.title(f'Diffuser Output\n {psnr_recon_l[ind_dm]:.2f} / {ssim_recon_l[ind_dm]:.4f}')
    plt.axis('off')
    
    plt.subplot(1, 3, 3)
    plt.imshow(pred_img)
    plt.title(f'Reconstructed\n {psnr_recon:.2f} / {ssim_recon:.4f}')
    plt.axis('off')
    
    plt.tight_layout()
    plt.savefig(f'results/comparison_{ind}.png', dpi=300)
    plt.close()
